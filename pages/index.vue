    ...
    <div class=" max-w-sm bg-white border border-gray-200 rounded-lg shadow inline-block m-5"
      ...
      <div class="flex flex-col items-center pb-10">
        <div class="flex mt-4 md:mt-6">
          ...
          <button v-if="simile[main.id - 1] == -100" @click="addToCart(main), compare()"
          ...
          <button v-if="simile[main.id - 1] == -1" @click="simile[main.id - 1] = -100, addToCart(main), compare()"
          ...
          <button v-if="simileFavourite[main.id - 1] == -1" @click="simileFavourite[main.id - 1] = -100, addToFavourite(main), compareFavourite()"
        ...
        <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentFill"/>
          ...
          <button v-if="simileFavourite[main.id - 1] >= 0" @click="simileFavourite[main.id - 1] = -100, deleteFromFavourite(main), compareFavourite()"
            class="py-2 px-4 ms-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 "><svg
              class="w-6 h-6 text-gray-800" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
...
function compare() {
  ...
  function compare1() {
    ...
    console.log(idArray) //список id в корзине

    simile.value = []
    for (let b = 1; b < 22; b++) {
      simile.value.push(idArray.indexOf(b))
    }
    console.log(simile.value); //0 /-1 нахождение карточки в корзине
  }
}
compare()
...
function compareFavourite() {
  ...
  console.log(locateFavourite.value)
  ...
  function compare1Favourite() {
    ...
    console.log(idArrayFavourite) //список id в корзине

    simileFavourite.value = []
    for (let b = 1; b < 22; b++) {
      simileFavourite.value.push(idArrayFavourite.indexOf(b))
    }
    console.log(simileFavourite.value); //0 /-1 нахождение карточки в корзине
  }
}
compareFavourite()
